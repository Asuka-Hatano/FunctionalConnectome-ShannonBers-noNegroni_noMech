function output = FunctionalConnectome_recalcForTau()
% Rabbit E-C coupling model by Shannon-Bers 
% The code is once coupled with myofilament force generation model by
% Negroni et al. and Cell-in-Gel mechanical equilibrium, 
% but currently commented OUT to keep it simple.
%
% Reference:
...
%y0  = [y10;y20;y30;y40;y50]; % With Negroni model
y0  = [y10;y20;y30;y40]; % Without Negroni model

%%%%% RANDOM SEED SEARCH and FILTERING ------------------------------------
%%%%% by checking agreement with Load-Free experimental data --------------
% Set the number of parameters to vary
nparamvary = 9;

%%%%% The following is codes for get random seeds for param sets ----------
%%%%% params_minmax is the input (instead of current input params_aps) ----
% nparamset = 100000;
...
% end
% params_base = lhsdesign(nparamset,nparamvary);
% params_minmax=zeros(nparamset,nparamvary);
% for iparamset = 1:nparamset
%     for iparamvary = 1:nparamvary
%         params_minmax(iparamset,iparamvary) = params_base(iparamset,iparamvary) * (paramMinMax(2,iparamvary) - paramMinMax(1,iparamvary)) + paramMinMax(1,iparamvary);
%     end
% end

%%%%% Read already calculated parameters-stableInitialCondition file ------
filename_pytable_in = "pyfinal_test20.csv";
filename_metrics_in = "Metrics_test20.csv";
py_table = readtable(filename_pytable_in);
nparamset = size(py_table,1);
params_aps = table2array(py_table(:,1:9));
yfinals    = table2array(py_table(:,10:49));

%%%%% Read already calculated results to obtain ORIGINAL IDs for paramsets ------
metrics_table = readtable(filename_metrics_in);
...
%%%%% Simulation setting
nbeat = 5; % Since mostly converged, we only need several beats. (but to check convergence, we need some) Originally set to 100.
niterconv = 100; % If not converge at first nbeat, iterate the process up to niterconv. 
% nb_save=1; % number of beats the waveform to be saved. Everything can be calculated with yfinal within seconds, I decided not to save anything.
nmetrics = 10; % =8 metrics +2 R for tau fitting
PCL=2000; % pacing cycle length. (ms)
% CL = 0.10; % closs linker is constant this time.

%%%%% LOAD FREE (LF) measures to compare with. (For filtering) ------------
SRCamaxLF= 0.5199;
SRCaRelLF= 0.1859;
SRCaVmxLF= 0.03382;
SRCaTauLF= 196.88;
...
CamaxLF  = 2.9833e-4;
CaTauLF  = 175.23;
% ampLF    = 0.0642; % for future use with mechanics

%%%%% Fit the important decay time constants in three ways and take the
%%%%% best one. The followings are for store the three taus.
...
npassed = 0;
for iparamset = 1:nparamset
    % 4 measures for checking convergence:
    APD     =zeros(1,nbeat);
    ...
    ifpassA = 0;
    %%%% This block is for checking LF (original params) condition
    % if(iparamset == 0) 
    %     params_c = ones(1,9);
    %     load("yfinal_0.mat");
    %     y0 = yfinal;
    ...
    % end
    p1 = params_c(1);  % ode solver accepts multiple scalars but not vectors 
    p2 = params_c(2);
    p3 = params_c(3);
    p4 = params_c(4);
    p5 = params_c(5);
    ...
    for lconv = 1:niterconv
        ...
        jstrNbeat = find(t>(nbeat-1)*PCL,1,"first"); % find array index where the last beat starts
        Vthresh = zeros(1,4); % prepare the array to calc for 4 kinds of APDs (with different thresholds)
        for ibeat = 1:nbeat
            ...
            try
                APD(ibeat) = t(indxend)-t(indxstr); % APD90. time course is needed for decide convergence
            catch
                APD(ibeat) = -100; % When waveform is apart from the assumption, it fails... This line avoid the code stopping here.
            end
        end
        APDinfo = zeros(1,6); % [APD90 APD80 APD50 APD25 APmax APmin] % The APDs of the LAST beat.
        APDinfo(5)=APmax;
        APDinfo(6)=APmin;
        for i=1:4
        ...
        if(nbeat > 1)
            ...
            if(all(FracError < 0.01))
                ifconvergeA = lconv*nbeat; % positive ifconvergence: Converged. the number indicates how long it took to converge.
                disp([num2str(iparamset),'   iteration',num2str(lconv*nbeat),'   FracError=', num2str(FracError)]);
                yfinals(iparamset,:) = y0.'; % update yfinals table
                break;
            else
                ifconvergeA = -max(FracError); % negative ifconvergence: Not converged. the number indicates how far it was from convergence.
            end
        end

    end

    %%%% After convergence, calculate the measures and indexes for this parameter sets
    % SRCaRel %
    ...
    % SRCaTau %
    % case 1: Fitting by Segmenting according to APD
...
%        [f,gof] = fit(xforfit,yforfit,'exp1', 'Exclude', exclude1);
    ...
    % case 2: Fitting from maximum value to the end of the beat (ORIGINAL)
    ...
    % case 3: Segmenting according to second derivative
    xforfit = t(indx_str:jend)-t(indx_str);    
...
%    dydt    = (yforfit(3:end)-yforfit(1:end-2))/(xforfit(3:end)-xforfit(1:end-2));
    ...
    hold on;
    plot(t(jstrNbeat:end),y(jstrNbeat:end,31),'.','color','black');
    
    % Chose the best fitted SRCaTau
    ...
    plot(xfitted,yfitted,'+','color',[221/256 170/256 51/256]);
    hold on;
    plot(t(jstrNbeat:end),y(jstrNbeat:end,38),'.','color','black');

    % Chose the best fit CaTau
...
%%%---CALC ADDITIONAL MEASURES to characterise the paramsets and its waveforms---%%%

    ncurrents = 13;
    integCurrents = zeros(1,ncurrents); 
    ...
    for i = 1:ncurrents
        peakCurrents(i)=max(currents(jstrNbeat:nstep,i));
    end
    Frdy = 96485;   % [C/mol]  
    ...
    metricsTF = zeros(1,8); % the above values are informative, but true-false expression is easier to filter.
    ...
    % end
    if(mod(iparamset,10)==0) % In case the code stops midway, save the results up to here.
        writematrix(output,"TempWriteMatrics.csv");
    end
end
% Final output
filename_pytable_out = "FCout_params_yfinal_test20_20240909.txt";
filename_metrics_out = "FCout_MetricsMeasuresAPDsCurrents_test20_20240909.csv";
filename_txtout      = "logfile_test20_20240909.txt";
...
writetable(output_table,filename_metrics_out,'Delimiter',',');
params_yfinals = [params_aps yfinals];
writematrix(params_yfinals,filename_pytable_out,'Delimiter',',');

writelines(["input params-yfinal: ", filename_pytable_in],filename_txtout);
writelines(["input metrics: ", filename_metrics_in],filename_txtout, "WriteMode","append");
writelines(["output params-yfinal: ", filename_pytable_out],filename_txtout, "WriteMode","append");
writelines(["output metrics: ", filename_metrics_out],filename_txtout, "WriteMode","append");
writelines("9 parameters were varied:　pCa, kfca, IbarNCX, ks, koCa, ec50SR, Vmax_SRCaP, Kmf, Kmr",filename_txtout, "WriteMode","append");
writelines("filtered by 7 metrics: CaSR-diastolic, CaSR-release, CaSR-Vmax, CaSR-uptake tau, CaT-diastolic, CaT-systolic, CaT-decay tau",filename_txtout, "WriteMode","append");
