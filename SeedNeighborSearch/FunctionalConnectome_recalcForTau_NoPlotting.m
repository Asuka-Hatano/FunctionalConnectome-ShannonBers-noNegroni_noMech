function output = FunctionalConnectome_recalcForTau_NoPlotting()
...
py_table = readtable("params_yfinal_passed_20240513.txt");
nparamset = size(py_table,1);

% m_table  = readtable("RecalcTau_metricspassed35K_20240509.csv");
% if(size(m_table,1) == nparamset)
%     disp(['nparamset ' num2str(nparamset)]);
% else
%     stop;
% end

params_base = table2array(py_table(:,1:9));
yfinals    = table2array(py_table(:,10:49));
%passConv   = table2array(m_table(:,1:3));
%metrics    = table2array(m_table(:,13:30));
%analyzed   = table2array(m_table(:,31:33));
%yfinal = load('result/yfinal_1.mat');
%y0 = yfinal;
...
CaTauLF  = 0.005715;
ampLF    = 0.0642;
%%--- MODIFICATION  MY PREVIOUS VERSION WAS MISTAKE < Tau is 1/b >----
SRCaTauLF = 1.0/SRCaTauLF;
CaTauLF   = 1.0/CaTauLF;

npassed = 0;
% Run programs changing parameters
parfor iparamset = 36:nparamset
    ...
    ifpassA = 0;
    params_aps = params_base(iparamset,:);
%    filename=strcat("./result_physiological/yfinal_",num2str(iparamset),".mat");
%    m = matfile(filename);
%    y0 = m.yfinal;
    y0_aps = yfinals(iparamset,:).';
    ...
    a_width = 0.3;
    npassed = 1;
    y0_b = zeros(mxloci,nvariables);
    params_b = zeros(mxloci,nparamvary);
    for igen = 1:ngen
        igen_params = zeros(mxloci,nseed,nparamvary);
        igen_results= zeros(mxloci,nseed);
        igen_y0s    = zeros(mxloci,nseed,nvariables);
        if(igen == 1 || npassed == 0)
            params_b(1,:) = params_aps;
            nloci = 1;
            y0_b(1,:)=y0_aps;
        else
            nloci = npassed;
        ...
        for iloci = 1:nloci
            seeds = lhsdesign(nseed,nparamvary);
            seeds = a_width * (seeds -0.5);
            for iseed = 1:nseed
                params_c = params_b(iloci,:) + seeds(iseed,:);
                for iparam = 1:nparamvary
                    params_c(iparam) = max(params_c(iparam), 0.3);
                end
                y0 = y0_b(iloci,:);
                ifpassA  = 0;
                p1 = params_c(1);
                ...
                p8 = params_c(8);
                p9 = params_c(9);
                numindex = iparamset*1000+(igen-1)*100+(iloci-1)*10+iseed-1;
                ...
                for lconv = 1:niterconv
                    ...
                    for ibeat = 1:nbeat
                        ...
                        for j=1:nsteps
                            ...
                            CaTmax(ibeat)=max(y(jstr:jend,38));
                        end
                    ...
                    if(nbeat > 1)
                        epsilon_APD=abs(APD(2:nbeat)-APD(1:nbeat-1))/200; % Normalization by the average value
                        epsilon_Ca =abs(CaTmax(2:nbeat)-CaTmax(1:nbeat-1))/0.0005;

                        if(all(epsilon_APD < 0.01))
                            epsilonfrac_APD = max(epsilon_APD);
                        else
                            epsilonfrac_APD = epsilon_APD(nbeat-1)/epsilon_APD(nbeat-2);
                        end
    
                        if(all(epsilon_Ca < 0.01))
                            epsilonfrac_Ca = max(epsilon_Ca);
                        else
                            epsilonfrac_Ca = epsilon_Ca(nbeat-1)/epsilon_Ca(nbeat-2);
                        end
                        ifconvergeA = max(epsilonfrac_APD, epsilonfrac_Ca);
                        if(abs(ifconvergeA) < 0.01)
                            disp([num2str(iparamset),'   iteration',num2str(lconv*10),'   epsilonCa=', num2str(epsilonfrac_Ca), '   epsilonAPD=',num2str(epsilonfrac_APD)])
                            ifconvergeA=lconv*nbeat;
                            break;
                        else
                            ifconvergeA = -ifconvergeA;
                        end
                    end
                ...
                % hold on;
                %    disp(["APD based fitting R = " num2str(arraySRfitR(2))]);
                ...
                % hold on;
                %    disp(["original fitting R = " num2str(arraySRfitR(1))]);
                ...
                % hold on;
                %    disp(["second derivative based fitting R = " num2str(arraySRfitR(3))]);
                ...
                % plot(xfitted,yfitted,"v","color",[187/256 85/256 102/256]);
                %    disp(["Original fitting R = " num2str(arrayCafitR(1))]);

                %APD-based
                %    indx_tAPD = find(t(jstrNbeat:jend)>20.0+APD(nbeat)*0.9,1,"First");
                xforfit = t(indx_str:indx_tAPD)-t(indx_str);
                ...
                % plot(xfitted,yfitted,'o','color',[0 68/256 136/256]);
                % hold on;
                %    disp(["APD based fitting R = " num2str(arrayCafitR(2))]);
                ...
                % plot(xfitted,yfitted,'+','color',[221/256 170/256 51/256]);
                % hold on;
                %    disp(["second derivative based fitting R = " num2str(arrayCafitR(3))]);
                % plot(t,y(:,38),'.','color','black');



                t_lb = t(jstrNbeat:jend);
                y_lb = y(jstrNbeat:jend,:);
                [currents] = calcCurrents(t_lb,y_lb,PCL,p1,p2,p3,p4,p5,p6,p7,p8,p9); % currents: [ I_Ca I_ncx J_SRCarel J_serca ];
                SRCaVmx= max(currents(:,3));

                indxstr = find(t_lb > t_lb(1)+35, 1, "first");
                indxend = find(t_lb > t_lb(1)+APD(nbeat)*0.8,1,"first");
                [ICamax,indxmax] = max(-currents(indxstr:indxend,1));
                xforfit = t_lb(indxstr+indxmax:indxend,1)-t_lb(indxstr+indxmax);
                yforfit = -currents(indxstr+indxmax:indxend,1)+currents(indxend,1);
                exclude1 = xforfit > APD(nbeat)*0.8-35;
                try
                    [f,gof] = fit(xforfit,yforfit,'exp1','Exclude',exclude1);
                    ICaTau  = - f.b;
                    ICaTfitR= gof.adjrsquare;
                catch
                    ICaTau = -1;
                    ICaTfitR = -1;
                end
                ...
                outputA = [numindex ifconvergeA ifpassA params_c metricsA metricsTF ICamax ICaTau APD(nbeat)];

                filename=strcat("./result/metrics",num2str(numindex),".txt");
                writematrix(outputA,filename,'Delimiter','\t');

                y0 = y(end,:);
                filename=strcat("./result/yfinal_",num2str(numindex),".mat");
                yfinal = y(end,:);
                myfinal = matfile(filename,'writable', true);
                myfinal.yfinal = yfinal;

                igen_params(iloci,iseed,:)=params_c;
                igen_results(iloci,iseed)=ifpassA;
                igen_y0s(iloci,iseed,:) = y0;
            end
        end
        npassed = sum(igen_results,"all");
        disp(['npassed = ', num2str(npassed)]);
        if(npassed > 15)
            a_width = a_width * 1.2;
        elseif(npassed < 5)
            a_width = a_width * 0.8;
        ...
        for iloci=1:nloci
            for iseed = 1:nseed
                if(igen_results(iloci,iseed)>0)
                    npassed = npassed +1;
                    params_b(npassed,:) = igen_params(iloci,iseed,:);
                    y0_b(npassed,:) = igen_y0s(iloci,iseed,:);
                    igen_results(iloci,iseed)=0;
                end
            end
            if(npassed == 10)
                break;
